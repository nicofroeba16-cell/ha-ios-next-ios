...
enum HomeAssistantOAuthError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .stateMismatch: "Der OAuth-Rückruf konnte nicht sicher zugeordnet werden."
        case let .authorizationDenied(message): message
        case let .tokenExchangeFailed(status):
            status.map { "Die Home-Assistant-Tokens konnten nicht abgerufen werden (HTTP \($0))." }
                ?? "Die Home-Assistant-Tokens konnten nicht abgerufen werden."
        }
    }
}
